...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    await db.execute(
        ...
        ', ${EpisodeColumns.epsdWork.value} TEXT)');
            await db.execute(
        'CREATE TABLE $logTableEpisode (${EpisodeColumns.id.value} INTEGER PRIMARY KEY, ${EpisodeColumns.name.value} TEXT, ${EpisodeColumns.typeEpisode.value} TEXT,${EpisodeColumns.operation.value} TEXT )');
    await db.execute(
 ...
 // }
  ...
  Future<List<Map<String, dynamic>>?> queryAllRowsWhereOR(
    ...
    Database? db = await instance.database;
    return await db
        ?.query(tableName, where: '$column1 = ? OR $column1 = ?', whereArgs: [value1, value2]);
  }
